...
void StateGameOverSFML::Init() 
{
    ...
    gameOverText.setOrigin(gameOverText.getLocalBounds().left+gameOverText.getLocalBounds().width/2.0f,
    ...
    restartText.setOrigin(restartText.getLocalBounds().left+restartText.getLocalBounds().width/2.0f,
...
void StateGameOverSFML::RestartGame()
{
    ...
    getPlayerSpawn().GetX(), context->map->GetSpawnsLayer()->getPlayerSpawn().GetY(),
     "Player", 10, 10, 4, 10);
