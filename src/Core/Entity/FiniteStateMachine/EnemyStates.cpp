...
bool MoveWithCollision(Enemy *e, CollisionLayer *cl, float vx, float vy,
                        std::unique_ptr<Player> &player_, int dt)
{
    if (vx == 0 && vy == 0)
    {
        return false;
    ...
    for (long unsigned int i = 0; i < cb.size(); i++)
    {
        ...
        if (posX + cbEnemy->GetWidth() >= cb[i].GetX() && cb[i].GetX() +
            cb[i].GetWidth() >= posX)
        {
            //Detection collision axe Y
            if (posY + cbEnemy->GetHeight() >= cb[i].GetY() && cb[i].GetY() +
                cb[i].GetHeight() >= posY)
            {
                iscolliding = true;
            }
    ...
    std::vector<std::shared_ptr<CollisionBox>> cbEnemies = 
                                                cl->GetCollisionBoxesEnemy();
    ...
    for (long unsigned int i = 0; i < cbEnemies.size(); i++)
    {
        if (e->GetCollisionBox()->GetId() != cbEnemies[i]->GetId())
        {
            ...
            {
                //Detection collision axe Y
                if (posY + e->GetCollisionBox()->GetHeight() >= 
                    cbEnemies[i]->GetY() && cbEnemies[i]->GetY() +
                    cbEnemies[i]->GetHeight() >= posY)
                {
    ...
    //Detection collision axe X
    if (posX + e->GetCollisionBox()->GetWidth() >= pX + offsetPlayer && pX +
        cbPlayer->GetWidth() - offsetPlayer >= posX)
    {
        //Detection collision axe Y
        if (posY + e->GetCollisionBox()->GetHeight() >= pY + offsetPlayer && pY
            + cbPlayer->GetHeight() - offsetPlayer >= posY)
        {
            iscolliding = true;
        }
    }
